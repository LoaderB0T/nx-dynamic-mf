...
export const getModulesToWatch = (
  ...
  const invalidNames = moduleNamesToWatch.filter(
    (p) => !modules.find((m) => m.name === p)
  );
  if (invalidNames.length > 0) {
    throw new Error(
      `Invalid module names for watch: ${invalidNames.join(', ')}`
    );
  }
  const invalidBuildNames = moduleNamesToWatch.filter(
    (p) => !modulesToBuild.find((m) => m.name === p)
  );
  if (invalidBuildNames.length > 0) {
    console.warn(
      `Invalid module names for watch, because they are served: ${invalidBuildNames.join(
        ', '
      )}`
    );
    invalidBuildNames.forEach((p) =>
      moduleNamesToWatch.splice(moduleNamesToWatch.indexOf(p), 1)
    );
  }
